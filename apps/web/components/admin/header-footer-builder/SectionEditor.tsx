      ...
      <div className="rounded border border-zinc-200 bg-zinc-50 p-3">
        ...
        <div className="mt-2 space-y-3">
          <div>
            ...
            <select value={p.width ?? DEFAULT_SECTION_LAYOUT.width} onChange={(e) => set({ width: e.target.value })} className={inputCls}>
          ...
          {(p.width ?? DEFAULT_SECTION_LAYOUT.width) === "boxed" && (
            <div>
              ...
              <NumberSlider label="" value={p.maxWidth ?? DEFAULT_SECTION_LAYOUT.maxWidth} onChange={(v) => set({ maxWidth: v })} min={400} max={1920} />
            </div>
          )}

          <div>
            <span className={labelCls}>Columns Gap</span>
            <select value={p.gapCol ?? DEFAULT_SECTION_LAYOUT.gapCol} onChange={(e) => set({ gapCol: Number(e.target.value) })} className={inputCls}>
          ...
          {p.height === "minHeight" && (
            <NumberSlider label="Minimum Height" value={p.minHeight ?? DEFAULT_SECTION_LAYOUT.minHeight} onChange={(v) => set({ minHeight: v })} min={0} max={1500} />
          )}
          ...
          <div className="flex items-center justify-between">
            <span className={labelCls}>Stretch Section</span>
            <button type="button" onClick={() => set({ stretchSection: !p.stretchSection })} className={`relative h-6 w-11 rounded-full transition-colors ${p.stretchSection ? "bg-zinc-900" : "bg-zinc-300"}`}>
          ...
          <p className="text-[10px] text-zinc-400">Stretch the section to the full width of the page using JS. <a href="#" className="text-zinc-500 underline">Learn more</a></p>
...
function StyleTab({ block, onChange, themeColors }: { block: SectionBlock; onChange: (p: SectionBlock["props"]) => void; themeColors?: Array<{ key: string; label: string; color: string }> }) {
  ...
  const bgPrefix = bgState === "hover" ? "hoverBg" : "";
  ...
  return (
    <div className="space-y-4">
          ...
          {(p as Record<string, unknown>)[getBgProp("bgType")] === "gradient" ? (
            <>
              ...
              <div>
                ...
                <select value={((p as Record<string, unknown>)[getBgProp("bgGradientType")] as string) || "linear"} onChange={(e) => set({ [getBgProp("bgGradientType")]: e.target.value })} className={inputCls}>
          ...
          <div>
            ...
            <div className="mt-1 grid grid-cols-4 gap-1">
              {(["borderRadiusTop", "borderRadiusRight", "borderRadiusBottom", "borderRadiusLeft"] as const).map((key, i) => (
                <input key={key} type="number" value={((p as Record<string, unknown>)[key] as number) || p.borderRadius || ""} onChange={(e) => set({ [key]: Number(e.target.value) || undefined })} className="w-full rounded border border-zinc-300 px-2 py-1.5 text-center text-xs" placeholder={["T", "R", "B", "L"][i]} min={0} />
              ))}
            </div>
            <div className="mt-0.5 flex justify-between px-1">
              {["T", "R", "B", "L"].map((l) => (
      ...
      <div className="rounded border border-zinc-200 bg-zinc-50 p-3">
        ...
        <div className="mt-2 space-y-3">
          <GlobalColorPicker label="Heading Color" value={p.headingColor} onChange={(c) => set({ headingColor: c || undefined })} allowClear paletteOverride={themeColors} />
      ...
      {openSection === "advanced" && (
        <div className="space-y-3 pb-3">
          ...
          <div>
            ...
            <input type="number" value={p.zindex ?? ""} onChange={(e) => onChange({ ...p, zindex: Number(e.target.value) || undefined })} placeholder="Auto" className={inputCls} />
          </div>
      ...
      {openSection === "responsive" && (
        <div className="space-y-3 pb-3">
          ...
          <div className="border-t border-zinc-200 pt-3">
            ...
            <p className="mt-1 text-[10px] text-zinc-400">Responsive visibility will take effect only on <span className="text-zinc-500 underline">preview mode</span> or live page, and not while editing in Elementor.</p>
          </div>
